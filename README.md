...
```text
KeepBright-1.4.0-macOS-arm64.dmg
```
...
dist/KeepBright-版本号-macOS-arm64.dmg
```
...
偏好设置窗口中可以配置启动后是否自动开启保持亮屏、自定义保持时长、是否每天自动检查更新，以及低电量时是否自动关闭保持亮屏。电池保护只在使用电池供电时触发，连接电源时不会自动关闭。
...
当 Mac 使用电池供电，并且电量低于或等于你在偏好设置里选择的阈值时，Keep Bright 会自动关闭保持亮屏。连接电源时不会触发。
