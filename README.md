...
```text
KeepBright-1.6.0-macOS-universal.dmg
```
...
偏好设置窗口分为“常规、计时、电池、更新、通知、关于”。其中可以配置防睡眠模式、菜单栏显示模式、全局快捷键、启动后是否自动开启保持亮屏、自定义保持时长、是否每天自动检查更新、通知偏好、低电量保护模式、电量阈值，以及插电后是否自动恢复保持亮屏。电池保护只在使用电池供电时触发，连接电源时不会自动关闭。
