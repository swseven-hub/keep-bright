...
```text
KeepBright-1.7.1-macOS-universal.dmg
```
...
- 启动后自动开启保持亮屏
- 支持自动化规则：指定 App、全屏、外接显示器、连接电源时自动保持亮屏
...
- 支持更接近系统设置风格的侧边栏偏好设置窗口
- 偏好设置内置分区图标、页面标题图标和更清晰的 Liquid Glass 分组层次
...
偏好设置窗口使用类似系统设置的侧边栏布局，分为“常规、自动化、计时、电池、更新、通知、关于”。其中可以配置防睡眠模式、菜单栏显示模式、全局快捷键、启动后是否自动开启保持亮屏、自动化触发规则、自定义保持时长、是否每天自动检查更新、通知偏好、低电量保护模式、电量阈值，以及插电后是否自动恢复保持亮屏。电池保护只在使用电池供电时触发，连接电源时不会自动关闭。

自动化规则默认关闭，避免升级后改变既有使用习惯。你可以在“偏好设置... > 自动化”中开启这些场景：

- 指定 App：当前台 App 名称或 Bundle ID 匹配列表时自动开启，例如 Keynote、PowerPoint、Zoom、腾讯会议、QuickTime Player。
- 全屏：当前台 App 拥有全屏窗口时自动开启，适合演示、观影和全屏会议。
- 外接显示器：连接外接显示器或投影仪时自动开启。
- 连接电源：接入电源适配器时自动开启，拔电后恢复。

如果 Keep Bright 是被自动化开启的，当所有自动化条件都不再满足时会自动关闭。若你在自动化条件仍满足时手动关闭，应用会暂停自动开启，直到当前条件结束后再恢复监听。
...
- `Sources/KeepBright/AutomationManager.swift`：自动化规则检测和触发状态管理
...
不会。自动化只会在规则满足且保持亮屏当前关闭时自动开启。如果你手动开启，自动化不会接管并在规则结束后替你关闭；如果你手动关闭自动化开启的保持亮屏，应用会暂停当前自动化触发，直到触发条件结束。
